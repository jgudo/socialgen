...
const Info = () => {
  const { profile, isOwnProfile } = useSelector((state: IRootState) => ({
    profile: state.profile.data,
    isOwnProfile: state.auth?.username === state.profile.data?.username
  }));
  const history = useHistory();
  useDocumentTitle(`Info - ${profile?.username || 'Profile'} | Foodie`);
