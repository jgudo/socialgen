...
import { appendNewMessage, closeChat, getMessagesSuccess, minimizeChat } from "~/redux/slice/chatSlice";
...
const ChatBox: React.FC<IProps> = ({ user, target }) => {
  ...
  useEffect(() => {
    ...
    socket.on('newMessage', (message: IMessage) => {
      dispatch(appendNewMessage({ target: target.username, message}));
  ...
  const fetchMessages = async () => {
    try {
      ...
      dispatch(getMessagesSuccess({ target: target.username, messages: messages.reverse()}));
  ...
  return (
    <div className={`bg-white dark:bg-indigo-1100 p-3 relative w-full h-full laptop:w-20rem laptop:shadow-lg laptop:rounded-t-xl laptop:bottom-0 laptop:right-24 laptop:border laptop:border-gray-400 dark:border-gray-800`}>
      <div className="flex justify-between pb-3 border-b border-gray-200 dark:border-gray-800">
        <Link className="dark:text-indigo-400" to={`/user/${target.username}`}>
          <div className="flex items-center">
            <Avatar url={typeof target.profilePicture === 'string' ? target.profilePicture : target.profilePicture?.url} className="mr-2" />
            <div className="flex flex-col">
              {target.fullname && <p className="text-xs text-gray-600 dark:text-gray-500">{target.fullname}</p>}
      ...
      {/* --- MESSAGES BODY ---- */}
      <PerfectScrollbar className="min-h-80% laptop:min-h-24rem max-h-85% laptop:max-h-80 bg-gray-100 dark:bg-indigo-1000 overflow-y-scroll pb-4 relative">
                  ...
                  <div className="flex flex-col">
                    ...
                    >
                      <div className="flex">
                        {/* -- AVATAR --- */}
                        <Avatar
                          url={typeof msg.from.profilePicture === 'string' ? msg.from.profilePicture : msg.from.profilePicture?.url}
                          size="xs"
