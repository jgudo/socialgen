...
import { IChatItemsState, IError, IMessage, IRootState, IUser } from "~/types/types";
...
const Messages: React.FC<{ isAuth: boolean; }> = ({ isAuth }) => {
  ...
  const handleReadMessage = async (sender: IUser | IChatItemsState) => {
    try {
      dispatch(initiateChat(sender as IChatItemsState));
      setMessagesOpen(false);
  ...
  return (
    <div className="relative">
      <div onClick={toggleMessages}>
        <Badge count={count}>
          <BiMessageDetail className="messages-toggle text-2xl focus:outline-none dark:text-white" />
        </Badge>
      </div>
      {isMessagesOpen && (
